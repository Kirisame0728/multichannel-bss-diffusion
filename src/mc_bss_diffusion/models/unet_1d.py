...
class Upsample(nn.Module):
    def __init__(self, S):
        super().__init__()
        N = 2 ** 12
        self.resample = torchaudio.transforms.Resample(N,
                                                       N * S)  # I use 3**12 as an arbitrary number, as we don't care about the sampling frequency of the latents
...
class Downsample(nn.Module):
    def __init__(self, S):
        ...
        N = 2 ** 12
        self.resample = torchaudio.transforms.Resample(N,
                                                       N / S)  # I use 2**12 as an arbitrary number, as we don't care about the sampling frequency of the latents
...
class ResnetBlock(nn.Module):
    def __init__(
        ...
        for i in range(self.num_layers):
            self.H.append(Gated_residual_layer(dim_out, 5, 2 ** i,
                                               bias=bias))  # sometimes I changed this 1,5 to 3,5. be careful!!! (in exp 80 as far as I remember)
