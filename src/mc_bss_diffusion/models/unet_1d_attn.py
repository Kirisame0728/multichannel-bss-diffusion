...
class ConditionedSequential(nn.Module):
    def __init__(self, *modules):
        super().__init__()
        if len(modules) == 1 and not isinstance(modules[0], nn.Module):
            modules = tuple(modules[0])  # generator / iterable
...
class UNet1dAttn(nn.Module):
    ...
    def get_channels(
        ...
        message = f"Expected context with {num_channels} channels at index {channels_id}, got {channels.shape[1]}"
        return channels
...
class UNetConditional1d(UNet1dAttn):
    """
