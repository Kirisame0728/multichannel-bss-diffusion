...
class Learner:
    def __init__(
        ...
        if self.log:
            ...
            # Record hyperparameters / metadata as text
            total_params = sum(p.numel() for p in self.model.parameters() if p.requires_grad)
            self.tb.add_text("hparams/total_params", str(total_params), global_step=0)

            # Dump a compact config snapshot
            cfg_lines = []
            cfg_lines.append(f"learning_rate: {getattr(self.args, 'lr', None)}")
            cfg_lines.append(f"audio_len: {getattr(self.args, 'audio_len', None)}")
            cfg_lines.append(f"sample_rate: {getattr(self.args, 'sample_rate', None)}")
            cfg_lines.append(f"batch_size: {getattr(self.args, 'batch_size', None)}")
            cfg_lines.append(f"sde_type: {getattr(self.args, 'sde_type', None)}")
            cfg_lines.append(f"architecture: {getattr(self.args, 'architecture', None)}")
            cfg_lines.append(
                f"num_steps: {getattr(self.args.inference, 'T', None) if hasattr(self.args, 'inference') else None}")
            if hasattr(self.args, "diffusion_parameters"):
                dp = self.args.diffusion_parameters
                cfg_lines.append(f"ro: {getattr(dp, 'ro', None)}")
                cfg_lines.append(f"sigma_max: {getattr(dp, 'sigma_max', None)}")
                cfg_lines.append(f"sigma_min: {getattr(dp, 'sigma_min', None)}")
                cfg_lines.append(f"Schurn: {getattr(dp, 'Schurn', None)}")
                cfg_lines.append(f"Snoise: {getattr(dp, 'Snoise', None)}")
                cfg_lines.append(f"Stmin: {getattr(dp, 'Stmin', None)}")
                cfg_lines.append(f"Stmax: {getattr(dp, 'Stmax', None)}")
            self.tb.add_text("hparams/config", "\n".join(cfg_lines), global_step=0)
    ...
    def _write_summary_sample(self, res, string):
        if not (self.log and self.tb is not None):
            return

        # 1) Spectrogram figure (matplotlib Figure expected)
        fig_stft = utils_logging.plot_spectrogram_from_raw_audio(res, self.args.stft)
        self.tb.add_figure(f"samples/spec/{string}", fig_stft, global_step=self.step, close=True)

        # 2) Audio (no need to write a wav file)
        audio0 = res[0].detach().cpu()  # first item in batch
        self.tb.add_audio(f"samples/audio/{string}", audio0.unsqueeze(0), global_step=self.step,
                          sample_rate=self.args.sample_rate)

        # 3) CQT figure (matplotlib Figure expected)
        fig_cqt = utils_logging.plot_CQT_from_raw_audio(res, self.args)
        self.tb.add_figure(f"samples/cqt/{string}", fig_cqt, global_step=self.step, close=True)
    ...
    def _write_summary(self):
        ...
        quantized_sigma = (sigma ** (1 / ro) - sigma_max ** (1 / ro)) * (self.n_bins - 1) / (
                    sigma_min ** (1 / ro) - sigma_max ** (1 / ro))
        quantized_sigma = quantized_sigma.astype(int)
        ...
        # loss-vs-sigma figure (matplotlib Figure expected)
        fig = utils_logging.plot_loss_by_sigma_train(sum_loss_in_bins, num_elems_in_bins, quantized_sigma_values[:-1])
        self.tb.add_figure("train/loss_by_sigma", fig, global_step=self.step, close=True)

        averaged_loss = float(np.mean(self.accumulated_losses))
        self.tb.add_scalar("train/averaged_loss", averaged_loss, global_step=self.step)

        mean_grad_norms = float(self.cum_grad_norms / max(num_elems_in_bins.sum(), 1.0) * self.args.batch_size)
        self.tb.add_scalar("train/mean_grad_norm", mean_grad_norms, global_step=self.step)
