...
def calc_sdr(estimation, origin, mask=None):
    ...
    eps = 1e-8
    true_power = np.sum(origin ** 2, 1) + eps  # (batch,)
    est_res = origin - estimation  # (batch, nsample)
    res_power = np.sum(est_res ** 2, 1) + eps  # (batch,)

    return 10 * np.log10(true_power) - 10 * np.log10(res_power)  # (batch,)
...
def calc_sdr_torch(estimation, origin, mask=None):
    ...
    eps = 1e-8
    true_power = torch.pow(origin, 2).sum(1) + eps  # (batch,)
    est_res = origin - estimation  # (batch, nsample)
    res_power = torch.pow(est_res, 2).sum(1) + eps  # (batch,)
